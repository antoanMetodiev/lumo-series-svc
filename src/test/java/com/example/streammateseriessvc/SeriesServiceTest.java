...
public class SeriesServiceTest {
    ...
    void testGetNext10Comments() {
        // Arrange
        int order = 2; // Трябва да бъде 2, защото (2 - 1) * 10 = 10
        UUID seriesId = UUID.randomUUID();
        int expectedOffset = (order - 1) * 10; // Трябва да бъде 10!

        List<Object[]> mockCommentData = List.of(
                new Object[]{UUID.randomUUID(), "Test comment 1", "user1", "User One", "img1.jpg", UUID.randomUUID(), 5.0, Instant.now()},
                new Object[]{UUID.randomUUID(), "Test comment 2", "user2", "User Two", "img2.jpg", UUID.randomUUID(), 4.0, Instant.now()}
        );

        when(seriesRepository.getNext10Comments(expectedOffset, seriesId)).thenReturn(mockCommentData);

        // Act
        List<SeriesComment> result = seriesService.getNext10Comments(order, seriesId);

        // Assert
        assertEquals(mockCommentData.size(), result.size());
        verify(seriesRepository, times(1)).getNext10Comments(expectedOffset, seriesId);

        // Допълнителна проверка дали данните са правилно мапнати
        assertEquals("Test comment 1", result.get(0).getCommentText());
